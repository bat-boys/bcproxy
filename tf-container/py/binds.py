from py.tfutils import tfeval, tfprint


WALKING: dict[str, str] = {
    "^[7": "@nw",
    "^[8": "@n",
    "^[9": "@ne",
    "^[u": "@w",
    "^[i": "@walking_middle",
    "^[o": "@e",
    "^[j": "@sw",
    "^[k": "@s",
    "^[l": "@se",
    "^[y": "@u",
    "^[h": "@d",
}

CASTING: dict[str, str] = {
    # tabs
    "^[1": "/python_call py.casting.tab 1",
    "^[2": "/python_call py.casting.tab 2",
    "^[3": "/python_call py.casting.tab 3",
    "^[4": "/python_call py.casting.tab 4",
    "^[5": "/python_call py.casting.tab 5",
    # casts without target
    # these calls are not type safe, make sure that casting
    "^[q": "/python_call py.casting.cast q",
    "^[w": "/python_call py.casting.cast w",
    "^[e": "/python_call py.casting.cast e",
    "^[r": "/python_call py.casting.cast r",
    "^[t": "/python_call py.casting.cast t",
    "^[a": "/python_call py.casting.cast a",
    "^[s": "/python_call py.casting.cast s",
    "^[d": "/python_call py.casting.cast d",
    "^[f": "/python_call py.casting.cast f",
    # targeted cast
    "^[Q": "/python_call py.casting.targeted_cast q",
    "^[W": "/python_call py.casting.targeted_cast w",
    "^[E": "/python_call py.casting.targeted_cast e",
    "^[R": "/python_call py.casting.targeted_cast r",
    "^[T": "/python_call py.casting.targeted_cast t",
    "^[A": "/python_call py.casting.targeted_cast a",
    "^[S": "/python_call py.casting.targeted_cast s",
    "^[D": "/python_call py.casting.targeted_cast d",
    "^[F": "/python_call py.casting.targeted_cast f",
    # misc
    "^[z": "@cast stop",
}

TARGETTING: dict[str, str] = {
    # normal party places
    "^[7": "/python_call py.party.target 1 1",
    "^[8": "/python_call py.party.target 1 2",
    "^[9": "/python_call py.party.target 1 3",
    "^[u": "/python_call py.party.target 2 1",
    "^[i": "/python_call py.party.target 2 2",
    "^[o": "/python_call py.party.target 2 3",
    "^[j": "/python_call py.party.target 3 1",
    "^[k": "/python_call py.party.target 3 2",
    "^[l": "/python_call py.party.target 3 3",
    # unknown party places
    "^[n": "/python_call py.party.target 0 0",
    "^[m": "/python_call py.party.target 0 1",
    "^[,": "/python_call py.party.target 0 2",
    "^[.": "/python_call py.party.target 0 3",
    "^[6": "/python_call py.party.target 1 0",
    "^[y": "/python_call py.party.target 2 0",
    "^[h": "/python_call py.party.target 3 0",
}


def bind(keys: dict[str, str]) -> None:
    for key in keys:
        tfeval(f"/bind {key} = {keys[key]}")


def walking(_s: str | None = None) -> None:
    bind(WALKING)
    tfprint("Walking binds")


def casting(_s: str | None = None) -> None:
    bind(CASTING)


def targetting(_s: str | None = None) -> None:
    bind(TARGETTING)
    tfprint("Targetting binds")
